...
const ViewFeedback = () => {
  ...
  const filteredFeedbacks = filter ? mockFeedbacks.filter(fb => fb.rating === filter) : mockFeedbacks;
  
  const averageRating = (mockFeedbacks.reduce((sum, fb) => sum + fb.rating, 0) / mockFeedbacks.length).toFixed(1);
  ...
  return (
    <div className={styles.feedback_container}>
      <div className={styles.insights}>
        <p><strong>Trung bình số sao:</strong> {averageRating} ⭐</p>
        {ratingCounts.map(({ star, count }) => (
        <p key={star}>{star}⭐: {count} đánh giá</p>
      ))}
      ...
      <div className={styles.feedback_list}>
        ...
        {filteredFeedbacks.map((feedback) => (
          <div
            key={feedback.reviewId}
            className={`${styles.feedback_item} ${selectedFeedback.reviewId === feedback.reviewId ? styles.active : ""}`}
            onClick={() => setSelectedFeedback(feedback)}
          >
            <p><strong>Review ID:</strong> {feedback.reviewId}</p>
      ...
      <div className={styles.feedback_detail}>
        <h2>Feedback Detail</h2>
        <p><strong>Review ID:</strong> {selectedFeedback.reviewId}</p>
        <p><strong>Customer ID:</strong> {selectedFeedback.customerId}</p>
        <p><strong>Staff ID:</strong> {selectedFeedback.staffId}</p>
        <p><strong>Vaccine ID:</strong> {selectedFeedback.vaccineId}</p>
        <p><strong>Appointment ID:</strong> {selectedFeedback.appointmentId}</p>
